...
exports.list = function(req, res) {
    Schedule.find().sort('-created').populate('user', 'displayName').populate('company').populate('day').exec(function(err, schedules) {
        if (err) {
            return res.status(400).send({
                message: errorHandler.getErrorMessage(err)
            });
...
/**
 * List of Schedules By Company
 */
exports.listByCompany = function(req, res) {
    Schedule.find({
        company: req.params.companyId
    }).sort('-created').populate('user', 'displayName').populate('company').populate('day').exec(function(err, fields) {
        if (err) {
            return res.status(400).send({
                message: errorHandler.getErrorMessage(err)
            });
