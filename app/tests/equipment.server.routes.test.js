...
describe('Equipment CRUD tests', function() {
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/equipments')
					...
					.end(function(equipmentSaveErr, equipmentSaveRes) {
						...
						agent.get('/equipments')
							.end(function(equipmentsGetErr, equipmentsGetRes) {
								// Handle Equipment save error
								if (equipmentsGetErr) done(equipmentsGetErr);

								// Get Equipments list
								var equipments = equipmentsGetRes.body;

								// Set assertions
								(equipments[0].user._id).should.equal(userId);
								(equipments[0].name).should.match('Equipment Name');
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/equipments')
					...
					.end(function(equipmentSaveErr, equipmentSaveRes) {
						...
						agent.put('/equipments/' + equipmentSaveRes.body._id)
							.send(equipment)
							.expect(200)
	...
	it('should be able to get a list of Equipments if not signed in', function(done) {
	...
	it('should be able to get a single Equipment if not signed in', function(done) {
		...
		// Save the Equipment
		equipmentObj.save(function() {
			request(app).get('/equipments/' + equipmentObj._id)
				.end(function(req, res) {
					// Set assertion
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/equipments')
					...
					.end(function(equipmentSaveErr, equipmentSaveRes) {
						...
						agent.delete('/equipments/' + equipmentSaveRes.body._id)
							.send(equipment)
							.expect(200)
	...
	it('should not be able to delete Equipment instance if not signed in', function(done) {
		...
		equipmentObj.save(function() {
			...
			request(app).delete('/equipments/' + equipmentObj._id)
			.expect(401)
			.end(function(equipmentDeleteErr, equipmentDeleteRes) {
				// Set message assertion
