...
angular.module('schedules').controller('SchedulesController', ['$scope', '$stateParams', '$location', 'Authentication', 'Schedules', 'Days',
    function($scope, $stateParams, $location, Authentication, Schedules, Days) {
        ...
        $scope.create = function() {
            ...
            $scope.days = _.sortBy($scope.days, 'number')
            _.forEach($scope.days, function(n, key) {
                if (key <= 4) {
                    var schedule = new Schedules({
                        company: $scope.companyId,
                        day: n._id,
                        startHour: $scope.semana.startHour.int,
                        endingHour: $scope.semana.endingHour.int,
                        valor: $scope.semana.valor
                    });
            ...
            _.forEach($scope.days, function(n, key) {
                if (key > 4) {
                    var schedule = new Schedules({
                        company: $scope.companyId,
                        day: n._id,
                        startHour: $scope.finde.startHour.int,
                        endingHour: $scope.finde.endingHour.int,
                        valor: $scope.finde.valor
                    });
                    schedules.push(schedule)
                }
            ...
            _.forEach(schedules,function(schedule,key){
                schedule.$save(function(response){
                    ...
                    if(responses == 7){
                      $scope.semana = {}
                      $scope.finde = {}
                      $location.path('companies/'+$scope.companyId+'/schedules');
                    }
